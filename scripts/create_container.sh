...
function create_container () {
    ...
    if ! echo "monitoring tolgee appsmith n8n illa owncloud nextcloud react cube chatwoot" | grep -w "$container_name" >/dev/null; then
    ...
    if [ -d "/var/lib/lxc/${container_name}" ]; then
        ...
        if echo "monitoring tolgee owncloud nextcloud react cube chatwoot" | grep -w "$container_name" >/dev/null; then
            ...
            "chatwoot")
                ...
                sleep 5
                sed -i "s|^FRONTEND_URL=.*|FRONTEND_URL=https://$container_name.$dom|g" /var/lib/lxc/$container_name/rootfs/home/chatwoot/chatwoot/.env
                sed -i "s|^ENABLE_ACCOUNT_SIGNUP=.*|ENABLE_ACCOUNT_SIGNUP=true|g" /var/lib/lxc/$container_name/rootfs/home/chatwoot/chatwoot/.env
